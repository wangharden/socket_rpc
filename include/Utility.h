...
typedef std::function<void(SOCKET, sockaddr_in)> UCallback;
struct ClientSock
{
    SOCKET socket;
    BYTE state = CLOSED;
...
public:
    ...
    */
    ...
    int SendParams(SOCKET s, Args&& ...args)
    {
        ...
        return this->Send(s, data.data(), (int)data.size(), 0);
    }
...
public:
    ...
    bool RegisterFunction(MsgCallback fun)
    {
        m_client.callback = fun;
        return true;
    }
    bool RegisterDisconnectFunction(UCallback fun)
    {
        m_client.disconnect_callback = fun;
        return true;
    }
    ...
    void Close() {
        ...
        if (m_client.disconnect_callback)
        {
            m_client.disconnect_callback(m_client.socket, m_client.sockaddr);
        }
    ...
    {
        std::string data = SockProto::Encode(args...);
        return this->Send(data.data(), (int)data.size(), 0);
    }
